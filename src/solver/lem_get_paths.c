...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/08/09 17:16:14 by pdamoune          #+#    #+#             */
/*   Updated: 2017/08/11 15:15:31 by pdamoune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		lem_get_path(t_list *path, t_list *links, t_room *room, int rayon)
{
	...
	while (links_tmp)
	{
		...
		if (room_tmp->weight < rayon - len
			&& !room_tmp->busy && room_tmp->position != START - 1)
		{
			room_tmp->busy = 1;
			len++;
			...
			ft_lstclr_last(&path);
			if (len-- && room_tmp->position != START - 1)
				room_tmp->busy = 0;
		}
...
int		lem_test_paths(t_list *path, t_room *room, int stop, int rayon)
{
	...
	while (++rayon <= (int)ft_lstlen(g_rooms))
	{
		...
		if (rayon == 2)
		{
			ft_lstadd_last(&g_multiple_paths, ft_lstptr(ft_lstdup(g_paths)));
			break ;
		}
		multiple = NULL;
		lem_multiple_paths(&multiple, g_paths, rayon);
		lem_clr_path(g_rooms);
		if ((int)ft_lstlen(g_multiple_paths) >= stop || g_ants < rayon)
			break ;
	}
	ft_lstptrdel(&path);
	return (1);
